...
class WindowAttentionEMA(nn.Module):
    r""" Window based multi-head self attention (W-MSA) module with relative position bias.
    It supports both of shifted and non-shifted window.
    Args:
        dim (int): Number of input channels.
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.,
        ...
        if args.ema == 'ema':
            self.move = MultiHeadEMA(embed_dim=dim, ndim=args.ndim, bidirectional=True)
        elif args.ema == 'ssm_2d':
            self.move = TwoDimensionalSSM(embed_dim=dim, ndim=args.ndim, L=window_size[0] ** 2, args=args)
    ...
    def forward(self, x, mask=None):
        ...
        qk = self.qk_proj(x_ema).reshape(B_, N, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        v = self.v_proj(x).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)[0]
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
        ...
        if self.shift_size == 0:
            self.attn = WindowAttentionEMA(
                dim, window_size=to_2tuple(self.window_size), num_heads=num_heads,
                qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, is_LSA=is_LSA, args=args)
        else:
